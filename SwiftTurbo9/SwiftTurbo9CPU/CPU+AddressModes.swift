...
private extension CPU {
    ...
    private func inh() -> ExtraClockCycles {
        0
    }
    ...
    private func imm8() -> ExtraClockCycles {
        addressAbsolute = PC
        PC = PC &+ 1
    ...
    /// This means that the program counter will already be pointing to this address.
    private func imm16() -> ExtraClockCycles {
        addressAbsolute = PC
        PC = PC &+ 2
    ...
    private func dir() -> ExtraClockCycles {
    ...
    private func rel8() -> ExtraClockCycles {
    ...
    private func rel16() -> ExtraClockCycles {
    ...
    private func ext() -> ExtraClockCycles {
    ...
    private func ind() -> ExtraClockCycles {
        ...
        if postByte & 0x80 == 0 {
            ...
            case 13: // direct constant offset from program counter 16-bit
                ...
                let result = Int(PC) &+ Int(nextValue)
                effectiveAddress = UInt16(truncatingIfNeeded: result)
            case 20, 24: // indirect constant offset from register (2s complement - no offset) 8 bit offset
                let nextValue = Int8(bitPattern: readByte(PC))
                PC = PC &+ 1
                let result = Int(registerValue) &+ Int(nextValue)
                effectiveAddress = UInt16(truncatingIfNeeded: result)
            ...
            case 21: // indirect B accumulator offset from register (2s complement offset)
                effectiveAddress = readWord(readWord(UInt16(Int(registerValue) &+ Int(B))))
            case 22: // indirect A accumulator offset from register (2s complement offset)
                effectiveAddress = readWord(readWord(UInt16(Int(registerValue) &+ Int(A))))
            case 27: // indirect D accumulator offset from register (2s complement offset)
                effectiveAddress = readWord(readWord(UInt16(Int(registerValue) &+ (Int(A) << 8 | Int(B)))))
